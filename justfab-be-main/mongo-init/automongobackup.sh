...
MONGOCONTAINER="mongo_smk"
DOCKERPORT="27017"
HOSTBACKUPDIR="/home/smk/app/backups"
...
# Email Address to send mail to? (user@domain.com)
 MAILADDR="phoenix23590@gmail.com"
...
DOHOURLY="no"
HOURLYRETENTION=24
# Add this line to the variable declarations section
ALWAYS_FULL_HOURLY="true"
...
ENABLE_FTP="true"
FTP_HOST="172.96.191.113"
FTP_USER="backupsmorning"
FTP_PASS="jWf!54nU1sF@"
FTP_BASE_DIR="/public_html/smk"
...
if [ "$DBUSERNAME" ]; then
    ...
    elif [ "$REQUIREDBAUTHDB" = "admin" ]; then
        OPT="$OPT --authenticationDatabase=admin"
    fi
fi
...
if [ "$DOHOURLY" == "yes" ]; then
  if [ "$ALWAYS_FULL_HOURLY" == "true" ]; then
    QUERY=""
  else
...
dbdump () {
    if [ "$ISDOCKER" == "yes" ]; then
          ...
          if [ -n "$QUERY" ]; then
              docker exec $MONGOCONTAINER mongodump --quiet --host=127.0.0.1 --port=$DOCKERPORT --out="$1" $OPT -q "$QUERY"
              MDUMPSTATUS=$?
          else
              docker exec $MONGOCONTAINER mongodump --quiet --host=127.0.0.1 --port=$DOCKERPORT --out="$1" $OPT
              MDUMPSTATUS=$?
          fi
    else
      if [ -n "$QUERY" ]; then
...
function select_secondary_member {
    ...
    mongo_output=$(mongosh --quiet --host $DBHOST:$DBPORT --eval 'rs.conf().members.forEach(function(x){ print(x.host) })' $OPTSEC 2>&1) || true

    # Log output for debugging
    echo "MongoDB Output: $mongo_output"
    if [[ $? -ne 0 || "$mongo_output" == *"not running with --replSet"* ]]; then
        echo "WARNING: Unable to retrieve replica set members. Proceeding with primary ${DBHOST}:${DBPORT}."
        return
    fi

    # Return list of with all replica set members
    # shellcheck disable=SC2086
    members=( $(mongosh --quiet --host $DBHOST:$DBPORT --eval 'rs.conf().members.forEach(function(x){ print(x.host) })' $OPTSEC ) )

    # Check each replset member to see if it's a secondary and return it.
    if [ ${#members[@]} -gt 1 ]; then
        for member in "${members[@]}"; do

            is_secondary=$(mongosh --quiet --host "$member" --eval 'rs.isMaster().secondary' $OPTSEC )
...
# Function to upload backup file via FTP
ftp_upload() {
    ...
    cd $FTP_BASE_DIR;
    mkdir -p $ftp_dir || echo "Directory already exists: $ftp_dir";
    cd $ftp_dir;
...
EOF
    if [ $? -eq 0 ]; then
        echo "FTP upload of $file_to_upload to $ftp_dir successful"
    else
        echo "FTP upload of $file_to_upload to $ftp_dir failed" >&2
    fi
}
...
if [[ $DOM = "01" ]] && [[ $DOMONTHLY = "yes" ]] && [[ "$HOUR" = "00" ]]; then
    ...
    if [[ $ENABLE_FTP = "true" ]]; then
        FTP_DIR="$FTP_BASE_DIR/monthly"
    fi

# Weekly Backup
elif [[ "$DNOW" = "$WEEKLYDAY" ]] && [[ "$DOWEEKLY" = "yes" ]] && [[ "$HOUR" = "00" ]]; then
    echo Weekly Backup
    echo
